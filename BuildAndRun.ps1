...
$tfmDir = $tfmDirs |
    Sort-Object { [version]((($_.Name -replace '^net', '') -split '-')[0]) } -Descending |
...
$appExecutable = Get-ChildItem -LiteralPath $outputDir -Filter *.exe -File |
    Where-Object { $_.Name -notmatch '^(apphost|createdump)\.exe$' } |
    Sort-Object Name |
    Select-Object -First 1
if (-not $appExecutable) {
    Write-Host "WARNING: No application executable found in $outputDir -- skipping run" -ForegroundColor Yellow
    exit 0
}
$runExecutableArgs = @('--executable', $appExecutable.Name)
...
if ($Detach) {
    ...
    & winapp run $outputDir @runExecutableArgs --detach --json
} else {
    $runArgs = @($outputDir) + $runExecutableArgs + @('--debug-output')
